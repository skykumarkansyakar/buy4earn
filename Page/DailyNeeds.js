...
export default class DailyNeeds extends Component {

  shouldComponentUpdate(prevProps) {
    if (prevProps.selectedId1 != null) {
      if (prevProps.selectedId1 == this.props.item.srid) {
        return true;
      }
      else {
  ...
  render() {
    return (
      ...
      <TouchableWithoutFeedback
        onPress={ this.props.navigationpageDailyNedds}
      >
  ...
  <Text style={{ color: 'white', fontSize: RFPercentage(1.5), backgroundColor: '#45CE30', borderRadius: 40, paddingLeft: wp('1.8%'), paddingRight: wp('1.8%') }}>
    ₹{this.props.item.discount} OFF
        ...
        <View style={{ flexDirection: 'row', height: hp('3%'), alignItems: 'center'}}>
          <View style={{ backgroundColor: 'white', paddingLeft: wp('0.5%'), paddingRight: ('1%'),height: hp('2.5%') }}>
            <Text style={{ fontSize: RFPercentage(2), color: '#45CE30', fontWeight: "bold" }}> ₹{this.props.item.rate}</Text>
          </View>
        ...
        : (
          <View style={{ alignItems: 'center', justifyContent: 'center', marginTop: hp('0.5%') }}>
            {this.props.item.AddButton==true ?
              <TouchableWithoutFeedback onPress={this.props.addbuttonSuggestion}>
                <View style={{
                  backgroundColor: 'red',
                  width: wp('46%'),
                  borderRadius: 10,
              ...
              (
               ...
               <View style={{ width: wp('46%'), height: hp('5%'), flexDirection: 'row', justifyContent: 'center', alignItems: 'center',borderRadius:2}}>
                {/* Minus Button */}
                <TouchableWithoutFeedback onPress={this.props.MinusButtonSuggestion} >
      ...
      <View style={{width:wp('22%'),borderWidth:1,borderColor:'red',height:hp('4.85%'),justifyContent:'center',alignItems:'center'}}>
         <TextInput 
          ...
          onChangeText={this.props.CartQuantityChangeSuggestion}
